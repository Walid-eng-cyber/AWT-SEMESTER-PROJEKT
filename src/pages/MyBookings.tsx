...
function BookingRow({ b, t }: { b: Booking; t: BookingTranslations }) {
  const statusLabel = b.status === 'confirmed' ? t.confirmed : b.status === 'pending' ? t.pending : t.cancelled
  ...
  return (
    <tr className="hidden sm:table-row border-b border-brand-border hover:bg-brand-surface/50 transition-colors">
      ...
      <td className="px-4 py-4">
        <span className={statusClass[b.status]}>{statusLabel}</span>
      </td>
      <td className="px-4 py-4">
        {b.status === 'cancelled' ? (
          <Link to="/rooms" className="inline-flex items-center gap-1 text-xs font-semibold text-brand-primary hover:underline">
            <RefreshCw size={12} /> {t.rebook}
          </Link>
        ) : b.status === 'pending' ? (
          <button className="text-xs font-semibold text-red-600 hover:underline">{t.cancel}</button>
        ) : (
          <Link to={`/rooms/${b.id}`} className="text-xs font-semibold text-brand-dark hover:underline">{t.view}</Link>
        )}
      </td>
    </tr>
  )
...
function BookingCard({ b, t }: { b: Booking; t: BookingTranslations }) {
  const statusLabel = b.status === 'confirmed' ? t.confirmed : b.status === 'pending' ? t.pending : t.cancelled
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 pb-24 lg:pb-8 overflow-y-auto">
          ...
          <div className="flex items-start justify-between mb-8 flex-wrap gap-4">
            <div>
              <p className="text-xs text-brand-muted mb-1">
                Room Management &rsaquo; <span className="text-brand-dark font-medium">{t.reservations}</span>
              </p>
              <h1 className="text-2xl font-bold text-brand-dark">{t.myBookings}</h1>
            </div>
            <Link to="/rooms" className="btn-primary text-xs py-2 px-4">+ {t.newBooking}</Link>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
            <div className="card p-5 sm:col-span-1">
              <p className="text-xs text-brand-muted uppercase tracking-wide mb-1">{t.upcomingThisWeek}</p>
              <p className="text-3xl font-bold text-brand-dark">02</p>
            </div>
            <div className="card p-5">
              <p className="text-xs text-brand-muted uppercase tracking-wide mb-1">{t.hoursReserved}</p>
              <p className="text-3xl font-bold text-brand-dark">12.5</p>
            </div>
            <div className="card p-5 bg-brand-dark text-white">
              <p className="text-xs text-brand-primary uppercase tracking-wide mb-1">{t.nextReservation}</p>
          ...
          <div className="card">
              ...
              <div className="flex gap-1">
                {(['upcoming', 'past'] as const).map(tabName => (
                  <button
                    ...
                    className={`px-4 py-1.5 text-sm font-medium rounded transition-colors ${tab === tabName ? 'bg-brand-dark text-white' : 'text-brand-muted hover:text-brand-dark'}`}
                  >
              ...
              <table className="hidden sm:table w-full text-sm">
                <thead>
                  <tr className="bg-brand-surface border-b border-brand-border">
                    <th className="text-left px-4 py-3 text-xs font-semibold text-brand-muted uppercase tracking-wide">{t.roomDetails}</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-brand-muted uppercase tracking-wide">{t.date}</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-brand-muted uppercase tracking-wide">{t.time}</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-brand-muted uppercase tracking-wide">{t.status}</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-brand-muted uppercase tracking-wide">{t.actions}</th>
                  </tr>
                </thead>
                <tbody>
                  {filtered.map(b => <BookingRow key={b.id} b={b} t={t} />)}
                  {filtered.length === 0 && (
                    <tr>
                      <td colSpan={5} className="text-center py-12 text-brand-muted text-sm">{t.noBookingsFound}</td>
                    </tr>
                  )}
                </tbody>
              </table>
            ...
            <div className="sm:hidden">
              {filtered.map(b => <BookingCard key={b.id} b={b} t={t} />)}
              {filtered.length === 0 && (
                <p className="text-center py-12 text-brand-muted text-sm">{t.noBookingsFound}</p>
              )}
            </div>
          </div>
        </main>
